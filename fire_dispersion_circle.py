...
def wildfire():
	...
	for step in range(num_steps + 10):
		print(temperatures_old[40][40])
		
		for i in range(size):
			temperatures_old[i][size - 1] = burn_temperature
		
		if step < ratio * 2:
			x = round(math.cos(math.pi * step / ratio) * ratio)
			y = round(math.sin(math.pi * step / ratio) * ratio)
			temperatures_old[int(x) + 60][int(y) + 60] = burn_temperature
			temperatures_old[int(x) + 61][int(y) + 61] = burn_temperature
			temperatures_old[int(x) + 60][int(y) + 61] = burn_temperature
			temperatures_old[int(x) + 61][int(y) + 60] = burn_temperature
