...
def wildfire():
	...
	for step in range(num_steps):
		...
		if step <  size:
			pto1 = [[37,96],[28,92],[7,93],[8,81]]
			t = float(step) / size
			p = bezier(pto1, t)
			temperatures_old[int(round(p[0]))][int(round(p[1]))] = burn_temperature
			#~ data[int(round(p[0]))][int(round(p[1]))] = 65530
			
		if step < size / 2:
			pto2 = [[37,96],[47,96],[67,98],[73,105]]
			t = float(step) / (size / 2) 
			p2 = bezier(pto2, t)
			temperatures_old[int(round(p2[0]))][int(round(p2[1]))] = burn_temperature
			#~ data[int(round(p2[0]))][int(round(p2[1]))] = 65530
			
		if step > size / 2 and step < size:
			pto = [[73,105],[84,105],[81,100],[88,95]]
		...
		if step > size and step < 2 * size:
			pto4 = [[88,95],[67,95],[72,73],[45,68]]
			pto5 = [[8,81],[32,78],[30,67],[45,68]]
			t = float(step - size) / size
			p2 = bezier(pto4, t)
			p3 = bezier(pto5, t)
			temperatures_old[int(round(p2[0]))][int(round(p2[1]))] = burn_temperature
			temperatures_old[int(round(p3[0]))][int(round(p3[1]))] = burn_temperature
			#~ data[int(round(p3[0]))][int(round(p3[1]))] = 65530
			#~ data[int(round(p2[0]))][int(round(p2[1]))] = 65530

		#~ if step > 2*size and step < 3*size:
			#~ pto5 = [[9,75],[32,75],[35,39],[81,90]]
