...
wood_1 = 100. # kg / m2
wood_2 = 70. # kg / m2
imagen = mpimg.imread('barreras2.png')
...
def wildfire():
	...
	for i in range(10):
		pto1 = [[51,72],[52,69],[57,68],[55,60]]
		p = bezier(pto1, float(i+1) / 10)
		temperatures_old[int(round(p[0]))][int(round(p[1]))] = burn_temperature
		
	
	for i in range(25):
		pto2 = [[35,38],[50,33],[52,23],[65,24]]
		pto3 = [[65,24],[78,30],[87,29],[94,27]]
		p2 = bezier(pto2, float(i) / 25)
		p3 = bezier(pto3, float(i) / 25)
		temperatures_old[int(round(p2[0]))][int(round(p2[1]))] = burn_temperature
		temperatures_old[int(round(p3[0]))][int(round(p3[1]))] = burn_temperature

	for i in range(18):
		pto1 = [[55,60],[43,57],[38,47],[35,38]]
		p = bezier(pto1, float(i+1) / 18)
		temperatures_old[int(round(p[0]))][int(round(p[1]))] = burn_temperature
	...
	for step in range(num_steps):
		...
		if step <  size:
			pto1 = [[49,72],[41,65],[19,50],[9,44]]
			t = float(step) / size
			p = bezier(pto1, t)
			temperatures_old[int(round(p[0]))][int(round(p[1]))] = burn_temperature
                        

		if step > size and step < 2 * size:
			pto4 = [[9,44],[18,24],[38,26],[55,16]]
			t = float(step - size) / size
			p2 = bezier(pto4, t)
			temperatures_old[int(round(p2[0]))][int(round(p2[1]))] = burn_temperature

		if step > 2 * size and step < 3 * size:
			pto4 = [[55,16],[77,12],[79,24],[96,22]]
			t = float(step - 2 * size) / size
			p2 = bezier(pto4, t)
			temperatures_old[int(round(p2[0]))][int(round(p2[1]))] = burn_temperature
