...
def wildfire():
	...
	for step in range(num_steps + 150):
		
		if step < size:
			temperatures_old[step][step/4] = burn_temperature
			
		if step > size and step < 2 * size:
			temperatures_old[2 * size - step][((step - size) / 4)+ (size / 4)] = burn_temperature 
			
		if step > 2 * size and step < 3 * size:
			temperatures_old[step - 2 * size][((step - 2 * size) / 4) + (size / 2) ] = burn_temperature

		if step > 3 * size and step < 4 * size:
			temperatures_old[4 * size - step][((step - 3 * size) / 4) + (size * 3/ 4)] = burn_temperature
			...
			for i in range(1, size - 1):
				...
				if data[j][i] == 0:
					if temperatures_new[j][i] > ignition_temperature:
						data[j][i] = step * 65530 / (num_steps + 150)
